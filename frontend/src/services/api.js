...
export const api = {
  ...
  getAdminHealthPanel: () => request('GET', '/admin/health-panel'),
  getAdminUsersKyc: (options = {}) => request('GET', '/admin/users/kyc', null, { query: options }),
  adminOverrideKyc: (id, body) => request('PATCH', `/admin/users/${id}/kyc`, body),
  getAdminCampaignKycGaps: (campaignId) => request('GET', `/admin/campaigns/${campaignId}/kyc-gaps`),
  getAdminFailedWebhookDeliveries: (options = {}) => request('GET', '/admin/webhooks/deliveries', null, { query: options }),
  adminRetryWebhookDelivery: (kind, id) => request('POST', `/admin/webhooks/deliveries/${kind}/${id}/retry`, {}),
